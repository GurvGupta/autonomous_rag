...
def main() -> None:
    ...
    try:
        ...
        st.warning("Could not create assistance, is the database running?")
        return
    
    #  Now going to load existing messages
    ...
    if last_message.get("role") == "user":
        ...
        with st.chat_message("assistant"):
            ...
            response = ""
            for item in auto_rag_assistant.run(question):
                response += item
    ...
    if auto_rag_assistant.knowledge_base:
        ...
        if add_url_button:
            if input_url is not None:
                ...
                if f"{input_url}_scrapped" not in st.session_state:
                    ...
                    if web_documents:
                        ...
                        st.sidebar.error("Could not read websites")
        ...
        if uploaded_file is not None:
            ...
            if f"{auto_rag_name}_uploadeed" not in st.session_state:
                reader = PDFReader()
